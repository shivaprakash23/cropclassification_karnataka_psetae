"""
Temporal Attention Encoder module for HLS L30 data

Adapted from the original TAE implementation
"""
...
class TemporalAttentionEncoder(nn.Module):
    def __init__(self, in_channels=128, n_head=4, d_k=32, d_model=None, n_neurons=[512, 128, 128], dropout=0.2,
                 T=360, len_max_seq=120, positions=None):
        """
        Sequence-to-embedding encoder for HLS L30 data.
        Args:
        ...
        self.name = 'TAE_dk{}_{}Heads_{}_T{}_do{}'.format(d_k, n_head, '|'.join(list(map(str, self.n_neurons))), T,
                                                          dropout)
        len_max_seq = 120  # Adjust this based on your HLS L30 data temporal sequence length
    ...
    def forward(self, x, dates):
        """
        Forward pass of the Temporal Attention Encoder
        Args:
            x: Input tensor of shape (batch_size, seq_len, d_model)
            dates: List of dates for positional encoding
        """
        ...
        if self.positions is None:
            # Convert dates list to tensor if it's a list
            if isinstance(dates, list):
                # Use sequence position as positional encoding
                src_pos = torch.arange(seq_len).unsqueeze(0).repeat(sz_b, 1).to(x.device)
            else:
                # Normalize dates to fit within the positional encoding range
                dates = (dates - dates.min()) / (dates.max() - dates.min() + 1e-8) * 24
                src_pos = dates.long().to(x.device)
        else:
            src_pos = self.positions.to(x.device)
