...
def run_training(params, base_dir, trial_num):
    ...
    # Calculate mlp1 and mlp2 dimensions correctly for S1 data
    # For S1 data, input_dim is 2 (2 bands)
    input_dim = 2
    
    # Define mlp1 dimensions
    mlp1 = [input_dim, 32, 64]  # Starting with input_dim (2 for S1)
    ...
    train_py_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'train.py')
    cmd = [
        ...
        '--lms', '180',  # Maximum sequence length for positional encoding
    ...
    try:
        ...
        if status == "SUCCESS":
            # If val_acc not found in stdout, try reading from overall.json
            if val_acc is None:
                try:
                    overall_path = os.path.join(trial_dir, "overall.json")
                    with open(overall_path, "r") as f:
                        overall = json.load(f)
                    val_acc = overall.get("Accuracy")
                    if val_acc is not None and val_acc <= 1.0:
                        val_acc *= 100  # Convert to percent if needed
                except Exception as e:
                    print(f"[ERROR] Could not read accuracy from overall.json: {e}")
                    return None
            if val_acc is not None:
                return {
                    'params': params,
...
def main():
    ...
    parser.add_argument('--res_dir', default='hypertuning_results_s2', help='Path to results directory')
    ...
    # Find best result
    # After all trials, loop through each trial dir and check test_metrics.json
    import glob
    import json
    best_test_acc = -1
    best_trial_num = None
    trial_dirs = glob.glob(os.path.join(args.res_dir, 'trial_*'))
    for trial_dir in trial_dirs:
        test_metrics_path = os.path.join(trial_dir, 'test_metrics.json')
        try:
            ...
            if test_acc is not None and test_acc > best_test_acc:
                ...
                # Extract only the trial number
                best_trial_num = int(os.path.basename(trial_dir).split('_')[-1])
        except Exception as e:
            continue
    if best_trial_num is not None:
        print(f"\nBest trial number (by test_accuracy): {best_trial_num}")
        print(f"Best test accuracy: {best_test_acc:.2f}%")
        
        # Read and display the hyperparameters from the best trial
        best_trial_dir = os.path.join(args.res_dir, f'trial_{best_trial_num}')
        try:
            with open(os.path.join(best_trial_dir, 'params.json'), 'r') as f:
                params = json.load(f)
            print("\nHyperparameters used in best trial:")
            for param, value in params.items():
                if param not in ['dataset_folder', 'val_folder', 'test_folder', 'epochs']:
                    print(f"  {param}: {value}")
        except Exception as e:
            print(f"Could not read parameters from best trial: {e}")
    else:
        print("No successful trials with test_metrics.json found.")
