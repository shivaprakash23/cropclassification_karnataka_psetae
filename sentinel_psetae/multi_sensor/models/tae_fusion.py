...
class TemporalAttentionEncoder(nn.Module):
    def __init__(self, in_channels=128, n_head=4, d_k=32, d_model=None, n_neurons=[512, 128, 128], dropout=0.2,
        ...
        elif positions == 'order':
            # Use T as the number of positions for date-based encoding
            positions = T + 1  # Add 1 to account for zero-based indexing
            self.name += '_orderPos'
        else:
    ...
    def forward(self, x, dates): # gets x and dates from pse. x = [batch sz, seq, features], dates = [batch s1, batch s2]
        ...
        if self.positions is None:
            ...
            # Ensure dates are within the valid range [0, T]
            src_pos = dates.long().to(x.device)
            src_pos = src_pos.clamp(min=0, max=self.T-1)
