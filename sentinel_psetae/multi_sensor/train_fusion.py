...
def custom_collate(batch):
    # Separate data into its components
    data_s1 = [item[0] for item in batch]  # S1 data
    data_s2 = [item[1] for item in batch]  # S2 data
    targets = [item[2] for item in batch]  # Labels
    dates = [item[3] for item in batch]    # Dates

    # Process S1 data
    s1_data = [d[0] for d in data_s1]
    s1_mask = [d[1] for d in data_s1]
    
    # Ensure all S1 sequences have length 16
    target_len = 16
    for i in range(len(s1_data)):
        curr_len = s1_data[i].size(0)
        if curr_len < target_len:
            # Pad with last observation
            pad_size = target_len - curr_len
            s1_data[i] = torch.cat([s1_data[i], s1_data[i][-1:].repeat(pad_size, 1, 1)], dim=0)
            s1_mask[i] = torch.cat([s1_mask[i], s1_mask[i][-1:].repeat(pad_size, 1)], dim=0)
        elif curr_len > target_len:
            # Take first target_len observations
            s1_data[i] = s1_data[i][:target_len]
            s1_mask[i] = s1_mask[i][:target_len]
    
    s1_data = torch.stack(s1_data)
    s1_mask = torch.stack(s1_mask)
    batched_s1 = (s1_data, s1_mask)

    # Process S2 data
    s2_data = [d[0] for d in data_s2]
    s2_mask = [d[1] for d in data_s2]
    
    # Ensure all S2 sequences have length 16
    for i in range(len(s2_data)):
        curr_len = s2_data[i].size(0)
        if curr_len < target_len:
            # Pad with last observation
            pad_size = target_len - curr_len
            s2_data[i] = torch.cat([s2_data[i], s2_data[i][-1:].repeat(pad_size, 1, 1)], dim=0)
            s2_mask[i] = torch.cat([s2_mask[i], s2_mask[i][-1:].repeat(pad_size, 1)], dim=0)
        elif curr_len > target_len:
            # Take first target_len observations
            s2_data[i] = s2_data[i][:target_len]
            s2_mask[i] = s2_mask[i][:target_len]
    
    s2_data = torch.stack(s2_data)
    s2_mask = torch.stack(s2_mask)
    batched_s2 = (s2_data, s2_mask)

    # Process dates with padding
    max_len_s1 = max(d[0].size(0) for d in dates)
    max_len_s2 = max(d[1].size(0) for d in dates)
    
    # Pad S1 dates
    padded_s1_dates = []
    for d in dates:
        s1_dates = d[0]
        if s1_dates.size(0) < max_len_s1:
            padding = torch.zeros(max_len_s1 - s1_dates.size(0), dtype=s1_dates.dtype)
            s1_dates = torch.cat([s1_dates, padding])
        padded_s1_dates.append(s1_dates)
    
    # Pad S2 dates
    padded_s2_dates = []
    for d in dates:
        s2_dates = d[1]
        if s2_dates.size(0) < max_len_s2:
            padding = torch.zeros(max_len_s2 - s2_dates.size(0), dtype=s2_dates.dtype)
            s2_dates = torch.cat([s2_dates, padding])
        padded_s2_dates.append(s2_dates)
    
    s1_dates = torch.stack(padded_s1_dates)
    s2_dates = torch.stack(padded_s2_dates)

    # Stack targets
    targets = torch.stack(targets)

    return batched_s1, batched_s2, targets, (s1_dates, s2_dates)
...
def main(config):
    ...
    for _, (train_loader, val_loader, test_loader) in enumerate(loaders):
        ...
        for epoch in range(1, config['epochs'] + 1):
            ...
            print('Loss {:.4f},  Acc {:.2f},  IoU {:.4f}'.format(val_metrics['val_loss'], val_metrics['val_accuracy'], val_metrics['val_IoU']))

            if config['fusion_type'] == 'softmax_learnable':
                with torch.no_grad():
                    w = torch.softmax(model.fusion_weights, dim=0)
                    print(f"Epoch {epoch} fusion weights → S1: {w[0]:.3f}, S2: {w[1]:.3f}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--minimum_sampling', default=16, type=int,
                        help='minimum time series length to sample')      
    parser.add_argument('--fusion_type', default='None', type=str,
                        help='level of multi-sensor fusion e.g. early, pse, tsa, softmax_avg, softmax_learnable')
