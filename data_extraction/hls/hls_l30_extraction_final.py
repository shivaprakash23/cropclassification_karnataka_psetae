...
"""
HLS L30 Extraction Script
This script extracts Harmonized Landsat Sentinel-2 (HLS) L30 data using Google Earth Engine.
"""
...
def get_collection(geometry, start_date, end_date, cloud_cover):
    ...
    print(f"Fetching L30 collection for dates: {start_date} to {end_date}")
    ...
    print(f"Filtering L30 collection for cloud cover: {cloud_cover}")
    
    # Select required bands for L30
...
def prepare_dataset(rpg_file, label_names=['CODE_GROUP'], id_field='ID_PARCEL', 
                   output_dir='l30_extraction_numpy_files', start_date='2024-09-01', 
                   end_date='2025-03-31', cloud_cover=20):
        ...
        try:
            ...
            for l in labels.keys():
                ...
                if lab_rpg[l][parcel_id] is None:
                    labels[l][parcel_id] = -1  # Use -1 as a placeholder for null values
                else:
                    # Store the value as is (could be string, number, etc.)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Extract HLS L30 data for parcels')
    ...
    # Make sure to pass the arguments in the correct order as defined in the function signature
    prepare_dataset(
